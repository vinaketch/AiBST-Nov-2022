...
```{r}
pca_iris <- prcomp( iris_features, scale. = T  )
pca_iris
```
...
```{r}
ggplot(pca_iris_dataframe,  aes(PC1,PC2) ) + geom_point()
...
```{r}
pca_iris_species <- ggplot(pca_iris_dataframe,aes(PC1,PC2, color=Species)  ) + geom_point()
